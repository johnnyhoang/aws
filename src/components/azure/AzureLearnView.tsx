...
export const AzureLearnView: React.FC<AzureLearnViewProps> = ({ onNavigateTab }) => {
  ...
  const filteredChapters = filterCategory === 'all'
    ? AZURE_CHAPTERS
    : AZURE_CHAPTERS.filter(ch => ch.category === filterCategory);
      ...
      <div className="space-y-3 pb-6 border-b border-slate-800">
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-3 text-xs bg-slate-900 border border-slate-800 px-3 py-1.5 rounded-full">
            ...
            <span className="font-mono font-bold text-blue-400">{completedCount}/{AZURE_CHAPTERS.length} bài ({progressPercent}%)</span>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
              ...
              return (
                <div
                  ...
                  className={`cursor-pointer px-3 py-2 rounded-lg transition-all flex items-start justify-between gap-2.5 ${
                    isSelected
                      ? 'bg-blue-950/40 text-blue-200 font-medium border-l-2 border-blue-500'
                      : 'hover:bg-slate-900/60 text-slate-400 hover:text-slate-200'
                  }`}
                >
                  <div className="space-y-0.5 min-w-0 flex-1">
                    <div className="text-xs font-medium truncate flex items-center gap-1.5">
                      <span className="font-mono text-blue-400 font-bold">#{ch.chapterNumber}</span>
          ...
          {onNavigateTab && (
            <div className="pt-3 border-t border-slate-800/80 space-y-1.5 text-xs">
              <button
                ...
                className="w-full py-2 px-3 rounded-lg hover:bg-slate-900/80 text-slate-400 hover:text-blue-300 transition-colors text-left flex items-center justify-between cursor-pointer"
              >
                <span>Luyện Thi Tình Huống Azure Architect</span>
              ...
              <button
                ...
                className="w-full py-2 px-3 rounded-lg hover:bg-slate-900/80 text-slate-400 hover:text-blue-300 transition-colors text-left flex items-center justify-between cursor-pointer"
              >
                <span>Lab Ma Trận Cân Bằng Tải & Ước Tính Chi Phí</span>
                <ChevronRight className="w-3.5 h-3.5 text-slate-500" />
              </button>
            </div>
          )}
        ...
        <article className="lg:col-span-8 space-y-8">
          ...
          <div className="space-y-4 pb-4 border-b border-slate-800">
            <div className="flex items-center justify-between gap-4">
              <div className="flex items-center gap-2 text-xs text-slate-400">
                <span className="font-mono text-blue-300 font-semibold uppercase">
          ...
          {currentChapter.hookStory && (
            <div className="border-l-2 border-blue-500/80 pl-4 py-1.5 space-y-1 bg-blue-950/10 rounded-r-lg">
          ...
          <blockquote className="border-l-2 border-blue-500/70 pl-4 py-2 text-xs sm:text-sm text-slate-300 leading-relaxed italic bg-slate-900/30 rounded-r-lg">
