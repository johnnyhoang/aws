...
import { RESEND_QUIZ_QUESTIONS } from '../../data/resend/resendQuizData';
import { RESEND_FLASHCARDS } from '../../data/resend/resendFlashcardsData';
import { ResendQuizQuestion, ResendFlashcard } from '../../types/resendModule';
import { 
  CheckCircle2, 
  XCircle, 
  Award
...
export const ResendTestView: React.FC = () => {
  ...
  const currentQuestion: ResendQuizQuestion = RESEND_QUIZ_QUESTIONS[currentQuizIndex];
  ...
  const handleNextQuestion = () => {
    ...
    if (currentQuizIndex < RESEND_QUIZ_QUESTIONS.length - 1) {
    ...
    <div className="max-w-4xl mx-auto px-4 sm:px-6 py-6 space-y-6 text-slate-300">
      ...
      <div className="flex items-center justify-between pb-4 border-b border-slate-800">
        <div>
          <h1 className="text-xl sm:text-2xl font-bold text-slate-100 flex items-center gap-2">
            ...
            <span>Đấu Trường Kiểm Tra: Resend API Expert</span>
          </h1>
        ...
        <div className="flex items-center gap-1 bg-slate-900 p-1 rounded-lg border border-slate-800 text-xs">
          ...
          >
            Thẻ Nhớ Thuật Ngữ ({RESEND_FLASHCARDS.length})
          </button>
        </div>
      </div>
      ...
      {activeTab === 'quiz' ? (
        <div className="space-y-6">
          ...
          <div className="flex items-center justify-between text-xs text-slate-400">
            <span className="font-mono text-rose-400 font-bold">
              Câu {currentQuizIndex + 1}/{RESEND_QUIZ_QUESTIONS.length}
            </span>
          ...
          <div className="space-y-3">
            <div className="border-l-2 border-rose-500 pl-4 py-1.5 text-xs sm:text-sm text-slate-300 italic bg-slate-900/30 rounded-r-lg">
              <strong className="text-rose-400 not-italic block mb-0.5">Tình huống thực tế:</strong>
          ...
          <div className="space-y-2.5">
            {currentQuestion.options.map((option) => {
              ...
              return (
                ...
                >
                  <span className="font-mono font-bold uppercase text-rose-400 mt-0.5">
                    {option.id}.
                  </span>
          ...
          {isAnswerSubmitted && (
            <div className="border-l-2 border-rose-400 pl-4 py-2 text-xs sm:text-sm text-slate-300 bg-slate-900/50 rounded-r-lg space-y-1">
              <span className="font-bold text-rose-300 block">Giải thích chi tiết:</span>
          ...
          <div className="pt-2 flex justify-end">
            {!isAnswerSubmitted ? (
              <button
                ...
                className="px-5 py-2 rounded-lg bg-rose-600 hover:bg-rose-500 disabled:opacity-40 text-white text-xs sm:text-sm font-semibold transition-all cursor-pointer"
              >
                Xác Nhận Đáp Án
              </button>
            ) : (
              <button
                ...
                className="px-5 py-2 rounded-lg bg-rose-600 hover:bg-rose-500 text-white text-xs sm:text-sm font-semibold transition-all cursor-pointer"
              >
                {currentQuizIndex < RESEND_QUIZ_QUESTIONS.length - 1 ? 'Câu Tiếp Theo →' : 'Làm Lại Bộ Đề ↺'}
              </button>
            )}
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {RESEND_FLASHCARDS.map((card: ResendFlashcard) => {
            ...
            return (
              <div
                ...
                className="p-4 rounded-xl bg-slate-900 border border-slate-800 hover:border-rose-500/50 transition-all cursor-pointer flex flex-col justify-between min-h-[180px]"
              >
                <div className="space-y-2">
                  ...
                  {!isFlipped ? (
                    ...
                    <div className="space-y-2 pt-1 text-xs">
                      ...
                      <p className="text-rose-300 font-semibold border-t border-slate-800 pt-1.5">
                        💡 {card.proTip}
                      </p>
                    </div>
                  )}
