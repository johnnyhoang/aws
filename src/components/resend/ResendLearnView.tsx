...
import { RESEND_CHAPTERS } from '../../data/resend/resendLessonsData';
import { ResendChapter, ResendCategory } from '../../types/resendModule';
import { 
  ChevronLeft, 
  ChevronRight, 
  Copy, 
...
interface ResendLearnViewProps {
  onNavigateTab?: (tab: 'learn' | 'test' | 'play') => void;
}

const CATEGORY_NAMES: Record<ResendCategory, string> = {
  overview_philosophy: 'Triết Lý & Tổng Quan',
  rest_api_sdks: 'REST API & SDKs',
  react_email_templates: 'React Email Templates',
  domain_verification_dns: 'Xác Thực Domain & DNS',
  idempotency_batch_limits: 'Idempotency & Batch Sending',
  webhooks_event_pipeline: 'Webhooks & Theo Dõi Sự Kiện',
  inbound_routing_parsing: 'Inbound Email Parsing',
  audiences_broadcasts: 'Audiences & Broadcasts',
  nextjs_server_actions: 'Next.js & Server Actions',
  deliverability_analytics: 'Deliverability & Inbox'
};

export const ResendLearnView: React.FC<ResendLearnViewProps> = ({ onNavigateTab }) => {
  ...
  const [selectedChapterId, setSelectedChapterId] = useState<string>(RESEND_CHAPTERS[0].id);
  const [copiedCodeIndex, setCopiedCodeIndex] = useState<string | null>(null);
  const [filterCategory, setFilterCategory] = useState<string>('all');

  const currentChapter: ResendChapter = RESEND_CHAPTERS.find(c => c.id === selectedChapterId) || RESEND_CHAPTERS[0];
  const currentIndex = RESEND_CHAPTERS.findIndex(c => c.id === currentChapter.id);
  const prevChapter = currentIndex > 0 ? RESEND_CHAPTERS[currentIndex - 1] : null;
  const nextChapter = currentIndex < RESEND_CHAPTERS.length - 1 ? RESEND_CHAPTERS[currentIndex + 1] : null;
  ...
  const filteredChapters = filterCategory === 'all'
    ? RESEND_CHAPTERS
    : RESEND_CHAPTERS.filter(ch => ch.category === filterCategory);

  const completedCount = RESEND_CHAPTERS.filter(ch => completedLessons.includes(ch.id)).length;
  const progressPercent = Math.round((completedCount / RESEND_CHAPTERS.length) * 100);
      ...
      <div className="space-y-3 pb-6 border-b border-slate-800">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="text-xs font-semibold text-rose-400 uppercase tracking-wider">
            Sách Chuyên Khảo: Resend & Modern Email APIs Mastery
          </div>
          ...
          <div className="flex items-center gap-3 text-xs bg-slate-900 border border-slate-800 px-3 py-1.5 rounded-full">
            ...
            <span className="font-mono font-bold text-rose-400">{completedCount}/{RESEND_CHAPTERS.length} bài ({progressPercent}%)</span>
          </div>
        </div>

        <h1 className="text-2xl sm:text-3xl font-bold text-slate-100 tracking-tight leading-tight">
          Làm Chủ Resend & Nền Tảng Email API Hiện Đại: Từ Code Đến Hộp Thư Khách Hàng
        </h1>
        <p className="text-sm sm:text-base text-slate-300 leading-relaxed max-w-4xl">
          Bộ giáo trình chuyên sâu 10 chương: Khám phá triết lý đột phá của Resend, thiết kế template đỉnh cao với React Email và Tailwind CSS, xác thực tên miền DNS đa tầng (SPF/DKIM/DMARC), chống gửi trùng lặp với Idempotency Keys, thiết lập Webhook pipeline theo dõi sự kiện thời gian thực và tích hợp chuyên sâu Next.js Server Actions.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
              ...
              return (
                <div
                  ...
                  className={`cursor-pointer px-3 py-2 rounded-lg transition-all flex items-start justify-between gap-2.5 ${
                    isSelected
                      ? 'bg-rose-950/40 text-rose-200 font-medium border-l-2 border-rose-500'
                      : 'hover:bg-slate-900/60 text-slate-400 hover:text-slate-200'
                  }`}
                >
                  <div className="space-y-0.5 min-w-0 flex-1">
                    <div className="text-xs font-medium truncate flex items-center gap-1.5">
                      <span className="font-mono text-rose-400 font-bold">#{ch.chapterNumber}</span>
          ...
          {onNavigateTab && (
            <div className="pt-3 border-t border-slate-800/80 space-y-1.5 text-xs">
              <button
                ...
                className="w-full py-2 px-3 rounded-lg hover:bg-slate-900/80 text-slate-400 hover:text-rose-300 transition-colors text-left flex items-center justify-between cursor-pointer"
              >
                <span>Luyện Thi Tình Huống Resend API</span>
              ...
              <button
                ...
                className="w-full py-2 px-3 rounded-lg hover:bg-slate-900/80 text-slate-400 hover:text-rose-300 transition-colors text-left flex items-center justify-between cursor-pointer"
              >
                <span>Lab Thử Nghiệm Gửi Mail & Sinh React Email</span>
                <ChevronRight className="w-3.5 h-3.5 text-slate-500" />
              </button>
            </div>
          )}
        ...
        <article className="lg:col-span-8 space-y-8">
          ...
          <div className="space-y-4 pb-4 border-b border-slate-800">
            <div className="flex items-center justify-between gap-4">
              <div className="flex items-center gap-2 text-xs text-slate-400">
                <span className="font-mono text-rose-300 font-semibold uppercase">
          ...
          {currentChapter.hookStory && (
            <div className="border-l-2 border-rose-500/80 pl-4 py-1.5 space-y-1 bg-rose-950/10 rounded-r-lg">
          ...
          <blockquote className="border-l-2 border-rose-500/70 pl-4 py-2 text-xs sm:text-sm text-slate-300 leading-relaxed italic bg-slate-900/30 rounded-r-lg">
            <strong className="text-rose-400 not-italic font-semibold block mb-0.5">Tóm tắt giá trị cốt lõi:</strong>
            ...
            {currentChapter.sections.map((section, sIdx) => (
              <section key={sIdx} className="space-y-3.5">
                <h3 className="text-base sm:text-lg font-bold text-slate-100 tracking-tight flex items-center gap-2">
                  <span className="w-1.5 h-4 bg-rose-400 rounded-full inline-block" />
                ...
                {section.mindsetShift && (
                  <div className="p-3.5 rounded-lg bg-slate-900/40 border-l-2 border-rose-500 space-y-2.5 text-xs">
                    <div className="font-bold text-rose-400 uppercase tracking-wide">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-slate-300">
                      ...
                      <div className="space-y-0.5">
                        <span className="font-semibold text-emerald-400 block">✅ Tư duy Resend Expert:</span>
                  ...
                  <div className="space-y-1 pt-1">
                    {section.codeBlock.title && (
                      <div className="flex items-center justify-between text-xs text-slate-400 px-1">
                        <span className="font-mono text-[11px] text-rose-300">{section.codeBlock.title}</span>
                        <button
          ...
          {currentChapter.practicalCommands && currentChapter.practicalCommands.length > 0 && (
            <div className="space-y-3 pt-4 border-t border-slate-800">
              <h3 className="text-sm font-bold text-slate-100 uppercase tracking-wide">
                Lệnh CLI Thực Hành Gửi Thư Resend
              </h3>
              ...
              <div className="space-y-3">
                {currentChapter.practicalCommands.map((cmd, cIdx) => (
                  <div key={cIdx} className="space-y-1 text-xs">
                    ...
                    <pre className="p-2.5 bg-slate-900/90 border border-slate-800/70 rounded-lg font-mono text-rose-300 overflow-x-auto">
          ...
          <div className="flex items-center justify-between pt-6 border-t border-slate-800 text-xs">
            {prevChapter ? (
              <button
                ...
                className="flex items-center gap-2 text-slate-400 hover:text-rose-300 transition-colors cursor-pointer group text-left"
              >
            ...
            {nextChapter ? (
              <button
                ...
                className="flex items-center gap-2 text-slate-400 hover:text-rose-300 transition-colors cursor-pointer group text-right"
              >
                <div>
