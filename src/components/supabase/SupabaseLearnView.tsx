...
import { SUPABASE_CHAPTERS } from '../../data/supabase/supabaseLessonsData';
import { SupabaseChapter, SupabaseCategory } from '../../types/supabaseModule';
import { 
  ChevronLeft, 
  ChevronRight, 
  Copy, 
...
interface SupabaseLearnViewProps {
  onNavigateTab?: (tab: 'learn' | 'test' | 'play') => void;
}

const CATEGORY_NAMES: Record<SupabaseCategory, string> = {
  overview_philosophy: 'Triết Lý & Tổng Quan',
  postgres_tables_views: 'Postgres SQL & Views',
  rls_security_policies: 'Row Level Security (RLS)',
  auth_oauth_mfa: 'Auth & Google OAuth',
  realtime_cdc_broadcast: 'Realtime Subscriptions',
  storage_buckets_presigned: 'Supabase Storage',
  edge_functions_deno: 'Edge Functions (Deno)',
  pgvector_ai_embeddings: 'pgvector & AI Search',
  webhooks_triggers_automation: 'Triggers & Webhooks',
  cli_migrations_branching: 'CLI & Local Dev'
};

export const SupabaseLearnView: React.FC<SupabaseLearnViewProps> = ({ onNavigateTab }) => {
  ...
  const [selectedChapterId, setSelectedChapterId] = useState<string>(SUPABASE_CHAPTERS[0].id);
  const [copiedCodeIndex, setCopiedCodeIndex] = useState<string | null>(null);
  const [isTocOpen, setIsTocOpen] = useState(false);

  const currentChapter: SupabaseChapter = SUPABASE_CHAPTERS.find(c => c.id === selectedChapterId) || SUPABASE_CHAPTERS[0];
  const currentIndex = SUPABASE_CHAPTERS.findIndex(c => c.id === currentChapter.id);
  const prevChapter = currentIndex > 0 ? SUPABASE_CHAPTERS[currentIndex - 1] : null;
  const nextChapter = currentIndex < SUPABASE_CHAPTERS.length - 1 ? SUPABASE_CHAPTERS[currentIndex + 1] : null;
  ...
  const completedCount = SUPABASE_CHAPTERS.filter(ch => completedLessons.includes(ch.id)).length;
  const progressPercent = Math.round((completedCount / SUPABASE_CHAPTERS.length) * 100);
    ...
    <div className="max-w-4xl mx-auto px-4 sm:px-6 py-6 space-y-8 text-slate-300">
      ...
      <div className="space-y-3 pb-6 border-b border-slate-800">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="text-xs font-semibold text-emerald-400 uppercase tracking-wider">
            Sách Chuyên Khảo: Supabase — Open Source Firebase & Postgres Mastery
          </div>
          ...
          <div className="flex items-center gap-3 text-xs bg-slate-900 border border-slate-800 px-3 py-1.5 rounded-full">
            ...
            <span className="font-mono font-bold text-emerald-400">{completedCount}/{SUPABASE_CHAPTERS.length} ({progressPercent}%)</span>
          </div>
        </div>

        <h1 className="text-2xl sm:text-3xl font-bold text-slate-100 tracking-tight leading-tight">
          Làm Chủ Supabase: Xây Dựng Fullstack Backend Đẳng Cấp Với Postgres & Realtime
        </h1>
        <p className="text-sm sm:text-base text-slate-300 leading-relaxed">
          Bộ giáo trình chuyên sâu 10 chương: Khám phá triết lý Open Source BaaS, thiết kế cơ sở dữ liệu quan hệ PostgreSQL chuẩn mực, làm chủ Row Level Security (RLS) bảo vệ từng dòng dữ liệu, xác thực Google OAuth, đồng bộ Realtime CDC, quản lý file Storage với Signed URLs, chạy Edge Functions và xây dựng ứng dụng AI với pgvector.
        </p>
      </div>
      ...
      <article className="space-y-8">
          ...
          <div className="flex flex-wrap items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-2 text-xs text-slate-400">
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-slate-900 border border-slate-800 text-slate-300 hover:text-emerald-300 hover:border-slate-700 transition-colors cursor-pointer font-semibold"
                title="Mở mục lục 10 chương sách"
              >
                <ListOrdered className="w-3.5 h-3.5 text-emerald-400" />
                <span>Mục Lục ({SUPABASE_CHAPTERS.length})</span>
              </button>

              <span className="text-slate-600 hidden sm:inline">|</span>
              <span className="font-mono text-emerald-300 font-semibold uppercase hidden sm:inline">
              ...
              <button
                ...
                className={`px-2.5 py-1.5 rounded-lg border transition-colors cursor-pointer flex items-center gap-1.5 ${
                  isPlaying && articleTitle === currentChapter.title
                    ...
                    : 'bg-slate-900 text-slate-400 border-slate-800 hover:text-emerald-300 hover:border-slate-700'
                }`}
        ...
        {currentChapter.hookStory && (
          <div className="border-l-2 border-emerald-500/80 pl-4 py-2 space-y-1 bg-emerald-950/10 rounded-r-lg">
        ...
        <blockquote className="border-l-2 border-emerald-500/70 pl-4 py-2.5 text-xs sm:text-sm text-slate-300 leading-relaxed italic bg-slate-900/30 rounded-r-lg">
          <strong className="text-emerald-400 not-italic font-semibold block mb-0.5">Tóm tắt giá trị cốt lõi:</strong>
          ...
          {currentChapter.sections.map((section, sIdx) => (
            <section key={sIdx} className="space-y-3.5">
              ...
              {section.mindsetShift && (
                <div className="p-3.5 rounded-lg bg-slate-900/40 border-l-2 border-emerald-500 space-y-2.5 text-xs">
                  <div className="font-bold text-emerald-400 uppercase tracking-wide">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-slate-300">
                    ...
                    <div className="space-y-0.5">
                      <span className="font-semibold text-emerald-400 block">✅ Tư duy Supabase Expert:</span>
                ...
                <div className="space-y-1 pt-1">
                  {section.codeBlock.title && (
                    <div className="flex items-center justify-between text-xs text-slate-400 px-1">
                      <span className="font-mono text-[11px] text-emerald-300">{section.codeBlock.title}</span>
                      <button
            ...
            <div className="space-y-3">
              {currentChapter.practicalCommands.map((cmd, cIdx) => (
                <div key={cIdx} className="space-y-1 text-xs">
                  ...
                  <pre className="p-2.5 bg-slate-900/90 border border-slate-800/70 rounded-lg font-mono text-emerald-300 overflow-x-auto">
        ...
        <div className="flex items-center justify-between pt-6 border-t border-slate-800 text-xs">
          {prevChapter ? (
            <button
              ...
              className="flex items-center gap-2 text-slate-400 hover:text-emerald-300 transition-colors cursor-pointer group text-left"
            >
          ...
          {nextChapter ? (
            <button
              ...
              className="flex items-center gap-2 text-slate-400 hover:text-emerald-300 transition-colors cursor-pointer group text-right"
            >
              <div>
      ...
      <BookTocDrawer
        ...
        chapters={SUPABASE_CHAPTERS}
        currentChapterId={currentChapter.id}
        completedLessonIds={completedLessons}
        onSelectChapter={handleSelectChapter}
        categoryNames={CATEGORY_NAMES}
        accentColorClass="text-emerald-400"
        bookTitle="Mục Lục: Supabase BaaS"
        onNavigateTab={onNavigateTab}
        testTabLabel="Luyện Thi Tình Huống Supabase Expert"
        playTabLabel="Lab Sinh Chính Sách RLS & Client Simulator"
      />
