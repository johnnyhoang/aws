    ...
    <div className="max-w-4xl mx-auto px-4 sm:px-6 py-6 space-y-8 text-slate-300">
      ...
      <article className="space-y-8">
          ...
          <div className="flex flex-wrap items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-2 text-xs text-slate-400">
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-slate-900 border border-slate-800 text-slate-300 hover:text-purple-300 hover:border-slate-700 transition-colors cursor-pointer font-semibold"
                title="Mở mục lục 10 chương sách"
              >
                <ListOrdered className="w-3.5 h-3.5 text-purple-400" />
                <span>Mục Lục ({OPEN_SOURCE_CHAPTERS.length})</span>
              </button>

              <span className="text-slate-600 hidden sm:inline">|</span>
              <span className="font-mono text-purple-300 font-semibold uppercase hidden sm:inline">
        ...
        {currentChapter.hookStory && (
          <div className="border-l-2 border-purple-500/80 pl-4 py-2 space-y-1 bg-purple-950/10 rounded-r-lg">
        ...
        <blockquote className="border-l-2 border-purple-500/70 pl-4 py-2.5 text-xs sm:text-sm text-slate-300 leading-relaxed italic bg-slate-900/30 rounded-r-lg">
      ...
      <BookTocDrawer
        ...
        chapters={OPEN_SOURCE_CHAPTERS}
        currentChapterId={currentChapter.id}
        completedLessonIds={completedLessons}
        onSelectChapter={handleSelectChapter}
        categoryNames={CATEGORY_NAMES}
        accentColorClass="text-purple-400"
        bookTitle="Mục Lục: Open Source Mastery"
        onNavigateTab={onNavigateTab}
        testTabLabel="Luyện Đề Trắc Nghiệm Tình Huống"
        playTabLabel="Lab Chọn License & PR Simulator"
      />
