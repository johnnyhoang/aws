    ...
    <div className="max-w-4xl mx-auto px-4 sm:px-6 py-6 space-y-8 text-slate-300">
      ...
      <div className="space-y-3 pb-6 border-b border-slate-800">
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-3 text-xs bg-slate-900 border border-slate-800 px-3 py-1.5 rounded-full">
            ...
            <span className="font-mono font-bold text-indigo-400">{completedCount}/{VERCEL_CHAPTERS.length} ({progressPercent}%)</span>
          </div>
        </div>
      ...
      <article className="space-y-8">
          ...
          <div className="flex flex-wrap items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-2 text-xs text-slate-400">
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-slate-900 border border-slate-800 text-slate-300 hover:text-indigo-300 hover:border-slate-700 transition-colors cursor-pointer font-semibold"
                title="Mở mục lục 10 chương sách"
              >
                <ListOrdered className="w-3.5 h-3.5 text-indigo-400" />
                <span>Mục Lục ({VERCEL_CHAPTERS.length})</span>
              </button>

              <span className="text-slate-600 hidden sm:inline">|</span>
              <span className="font-mono text-indigo-300 font-semibold uppercase hidden sm:inline">
        ...
        {currentChapter.hookStory && (
          <div className="border-l-2 border-indigo-500/80 pl-4 py-2 space-y-1 bg-indigo-950/10 rounded-r-lg">
        ...
        <blockquote className="border-l-2 border-indigo-500/70 pl-4 py-2.5 text-xs sm:text-sm text-slate-300 leading-relaxed italic bg-slate-900/30 rounded-r-lg">
      ...
      <BookTocDrawer
        ...
        chapters={VERCEL_CHAPTERS}
        currentChapterId={currentChapter.id}
        completedLessonIds={completedLessons}
        onSelectChapter={handleSelectChapter}
        categoryNames={CATEGORY_NAMES}
        accentColorClass="text-indigo-400"
        bookTitle="Mục Lục: Vercel Cloud"
        onNavigateTab={onNavigateTab}
        testTabLabel="Luyện Thi Tình Huống Vercel Expert"
        playTabLabel="Lab Cấu Hình vercel.json & DNS Simulator"
      />
