...
import { NEON_CHAPTERS } from '../../data/neon/neonLessonsData';
import { NeonChapter, NeonCategory } from '../../types/neonModule';
import { 
  ChevronLeft, 
  ChevronRight, 
  Copy, 
...
interface NeonLearnViewProps {
  onNavigateTab?: (tab: 'learn' | 'test' | 'play') => void;
}

const CATEGORY_NAMES: Record<NeonCategory, string> = {
  overview_architecture: 'Tổng Quan & Tách Rời',
  storage_compute_separation: 'Pageserver & S3 Engine',
  database_branching: 'Database Branching (CoW)',
  autoscaling_scale_to_zero: 'Autoscaling & Scale-to-Zero',
  connection_pooling_pgbouncer: 'Connection Pooling & PgBouncer',
  point_in_time_recovery_time_travel: 'Point-in-Time Recovery',
  serverless_driver_websockets: 'Neon Serverless Driver',
  neon_auth_data_api: 'Neon Auth & Data API',
  cli_terraform_api: 'CLI & Tự Động Hóa CI/CD',
  cost_performance_optimization: 'Tối Ưu Chi Phí & Hiệu Năng'
};

export const NeonLearnView: React.FC<NeonLearnViewProps> = ({ onNavigateTab }) => {
  ...
  const [selectedChapterId, setSelectedChapterId] = useState<string>(NEON_CHAPTERS[0].id);
  const [copiedCodeIndex, setCopiedCodeIndex] = useState<string | null>(null);
  const [isTocOpen, setIsTocOpen] = useState(false);

  const currentChapter: NeonChapter = NEON_CHAPTERS.find(c => c.id === selectedChapterId) || NEON_CHAPTERS[0];
  const currentIndex = NEON_CHAPTERS.findIndex(c => c.id === currentChapter.id);
  const prevChapter = currentIndex > 0 ? NEON_CHAPTERS[currentIndex - 1] : null;
  const nextChapter = currentIndex < NEON_CHAPTERS.length - 1 ? NEON_CHAPTERS[currentIndex + 1] : null;
  ...
  const completedCount = NEON_CHAPTERS.filter(ch => completedLessons.includes(ch.id)).length;
  const progressPercent = Math.round((completedCount / NEON_CHAPTERS.length) * 100);
    ...
    <div className="max-w-4xl mx-auto px-4 sm:px-6 py-6 space-y-8 text-slate-300">
      ...
      <div className="space-y-3 pb-6 border-b border-slate-800">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="text-xs font-semibold text-cyan-400 uppercase tracking-wider">
            Sách Chuyên Khảo: Neon Serverless Postgres Mastery
          </div>
          ...
          <div className="flex items-center gap-3 text-xs bg-slate-900 border border-slate-800 px-3 py-1.5 rounded-full">
            ...
            <span className="font-mono font-bold text-cyan-400">{completedCount}/{NEON_CHAPTERS.length} ({progressPercent}%)</span>
          </div>
        </div>

        <h1 className="text-2xl sm:text-3xl font-bold text-slate-100 tracking-tight leading-tight">
          Làm Chủ Neon & Serverless Postgres: Kiến Trúc Tách Rời, Branching & Co Giãn Vô Hạn
        </h1>
        <p className="text-sm sm:text-base text-slate-300 leading-relaxed">
          Bộ giáo trình chuyên sâu 10 chương: Giải mã kiến trúc tách rời Compute và Storage của Neon, làm chủ tính năng Database Branching (Copy-on-Write) cho CI/CD, cơ chế tự động ngủ đông Scale-to-Zero, giải quyết tắc nghẽn kết nối Serverless với PgBouncer, du hành thời gian với Point-in-Time Recovery và truy vấn siêu tốc qua WebSockets/HTTP Driver.
        </p>
      </div>
      ...
      <article className="space-y-8">
          ...
          <div className="flex flex-wrap items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-2 text-xs text-slate-400">
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-slate-900 border border-slate-800 text-slate-300 hover:text-cyan-300 hover:border-slate-700 transition-colors cursor-pointer font-semibold"
                title="Mở mục lục 10 chương sách"
              >
                <ListOrdered className="w-3.5 h-3.5 text-cyan-400" />
                <span>Mục Lục ({NEON_CHAPTERS.length})</span>
              </button>

              <span className="text-slate-600 hidden sm:inline">|</span>
              <span className="font-mono text-cyan-300 font-semibold uppercase hidden sm:inline">
              ...
              <button
                ...
                className={`px-2.5 py-1.5 rounded-lg border transition-colors cursor-pointer flex items-center gap-1.5 ${
                  isPlaying && articleTitle === currentChapter.title
                    ? 'bg-cyan-500/20 text-cyan-300 border-cyan-500/50 shadow-sm animate-pulse'
                    : 'bg-slate-900 text-slate-400 border-slate-800 hover:text-cyan-300 hover:border-slate-700'
                }`}
        ...
        {currentChapter.hookStory && (
          <div className="border-l-2 border-cyan-500/80 pl-4 py-2 space-y-1 bg-cyan-950/10 rounded-r-lg">
        ...
        <blockquote className="border-l-2 border-cyan-500/70 pl-4 py-2.5 text-xs sm:text-sm text-slate-300 leading-relaxed italic bg-slate-900/30 rounded-r-lg">
          <strong className="text-cyan-400 not-italic font-semibold block mb-0.5">Tóm tắt giá trị cốt lõi:</strong>
          ...
          {currentChapter.sections.map((section, sIdx) => (
            <section key={sIdx} className="space-y-3.5">
              ...
              {section.mindsetShift && (
                <div className="p-3.5 rounded-lg bg-slate-900/40 border-l-2 border-cyan-500 space-y-2.5 text-xs">
                  <div className="font-bold text-cyan-400 uppercase tracking-wide">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-slate-300">
                    ...
                    <div className="space-y-0.5">
                      <span className="font-semibold text-emerald-400 block">✅ Tư duy Neon Expert:</span>
                ...
                <div className="space-y-1 pt-1">
                  {section.codeBlock.title && (
                    <div className="flex items-center justify-between text-xs text-slate-400 px-1">
                      <span className="font-mono text-[11px] text-cyan-300">{section.codeBlock.title}</span>
                      <button
        ...
        {currentChapter.practicalCommands && currentChapter.practicalCommands.length > 0 && (
          <div className="space-y-3 pt-4 border-t border-slate-800">
            <h3 className="text-sm font-bold text-slate-100 uppercase tracking-wide">
              Lệnh Neon CLI (neonctl) Thực Hành
            </h3>
            ...
            <div className="space-y-3">
              {currentChapter.practicalCommands.map((cmd, cIdx) => (
                <div key={cIdx} className="space-y-1 text-xs">
                  ...
                  <pre className="p-2.5 bg-slate-900/90 border border-slate-800/70 rounded-lg font-mono text-cyan-300 overflow-x-auto">
        ...
        <div className="flex items-center justify-between pt-6 border-t border-slate-800 text-xs">
          {prevChapter ? (
            <button
              ...
              className="flex items-center gap-2 text-slate-400 hover:text-cyan-300 transition-colors cursor-pointer group text-left"
            >
          ...
          {nextChapter ? (
            <button
              ...
              className="flex items-center gap-2 text-slate-400 hover:text-cyan-300 transition-colors cursor-pointer group text-right"
            >
              <div>
      ...
      <BookTocDrawer
        ...
        chapters={NEON_CHAPTERS}
        currentChapterId={currentChapter.id}
        completedLessonIds={completedLessons}
        onSelectChapter={handleSelectChapter}
        categoryNames={CATEGORY_NAMES}
        accentColorClass="text-cyan-400"
        bookTitle="Mục Lục: Neon Serverless Postgres"
        onNavigateTab={onNavigateTab}
        testTabLabel="Luyện Thi Tình Huống Neon Serverless"
        playTabLabel="Lab Mô Phỏng Branching & Connection URI"
      />
