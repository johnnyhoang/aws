...
export const NeonLearnView: React.FC<NeonLearnViewProps> = ({ onNavigateTab }) => {
  ...
  const filteredChapters = filterCategory === 'all'
    ? NEON_CHAPTERS
    : NEON_CHAPTERS.filter(ch => ch.category === filterCategory);
      ...
      <div className="space-y-3 pb-6 border-b border-slate-800">
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-3 text-xs bg-slate-900 border border-slate-800 px-3 py-1.5 rounded-full">
            ...
            <span className="font-mono font-bold text-cyan-400">{completedCount}/{NEON_CHAPTERS.length} bài ({progressPercent}%)</span>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
              ...
              return (
                <div
                  ...
                  className={`cursor-pointer px-3 py-2 rounded-lg transition-all flex items-start justify-between gap-2.5 ${
                    isSelected
                      ? 'bg-cyan-950/40 text-cyan-200 font-medium border-l-2 border-cyan-500'
                      : 'hover:bg-slate-900/60 text-slate-400 hover:text-slate-200'
                  }`}
                >
                  <div className="space-y-0.5 min-w-0 flex-1">
                    <div className="text-xs font-medium truncate flex items-center gap-1.5">
                      <span className="font-mono text-cyan-400 font-bold">#{ch.chapterNumber}</span>
          ...
          {onNavigateTab && (
            <div className="pt-3 border-t border-slate-800/80 space-y-1.5 text-xs">
              <button
                ...
                className="w-full py-2 px-3 rounded-lg hover:bg-slate-900/80 text-slate-400 hover:text-cyan-300 transition-colors text-left flex items-center justify-between cursor-pointer"
              >
                <span>Luyện Thi Tình Huống Neon Serverless</span>
              ...
              <button
                ...
                className="w-full py-2 px-3 rounded-lg hover:bg-slate-900/80 text-slate-400 hover:text-cyan-300 transition-colors text-left flex items-center justify-between cursor-pointer"
              >
                <span>Lab Mô Phỏng Branching & Connection URI</span>
                <ChevronRight className="w-3.5 h-3.5 text-slate-500" />
              </button>
            </div>
          )}
        ...
        <article className="lg:col-span-8 space-y-8">
          ...
          {currentChapter.hookStory && (
            <div className="border-l-2 border-cyan-500/80 pl-4 py-1.5 space-y-1 bg-cyan-950/10 rounded-r-lg">
          ...
          <blockquote className="border-l-2 border-cyan-500/70 pl-4 py-2 text-xs sm:text-sm text-slate-300 leading-relaxed italic bg-slate-900/30 rounded-r-lg">
