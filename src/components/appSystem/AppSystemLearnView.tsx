    ...
    <div className="max-w-4xl mx-auto px-4 sm:px-6 py-6 space-y-8 text-slate-300">
      ...
      <div className="space-y-3 pb-6 border-b border-slate-800">
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-3 text-xs bg-slate-900 border border-slate-800 px-3 py-1.5 rounded-full">
            ...
            <span className="font-mono font-bold text-violet-400">{completedCount}/{APP_SYSTEM_CHAPTERS.length} ({progressPercent}%)</span>
          </div>
        </div>
      ...
      <article className="space-y-8">
          ...
          <div className="flex flex-wrap items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-2 text-xs text-slate-400">
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-slate-900 border border-slate-800 text-slate-300 hover:text-violet-300 hover:border-slate-700 transition-colors cursor-pointer font-semibold"
                title="Mở mục lục 10 chương sách"
              >
                <ListOrdered className="w-3.5 h-3.5 text-violet-400" />
                <span>Mục Lục ({APP_SYSTEM_CHAPTERS.length})</span>
              </button>

              <span className="text-slate-600 hidden sm:inline">|</span>
              <span className="font-mono text-violet-300 font-semibold uppercase hidden sm:inline">
        ...
        {currentChapter.hookStory && (
          <div className="border-l-2 border-violet-500/80 pl-4 py-2 space-y-1 bg-violet-950/10 rounded-r-lg">
        ...
        <blockquote className="border-l-2 border-violet-500/70 pl-4 py-2.5 text-xs sm:text-sm text-slate-300 leading-relaxed italic bg-slate-900/30 rounded-r-lg">
      ...
      <BookTocDrawer
        ...
        chapters={APP_SYSTEM_CHAPTERS}
        currentChapterId={currentChapter.id}
        completedLessonIds={completedLessons}
        onSelectChapter={handleSelectChapter}
        categoryNames={CATEGORY_NAMES}
        accentColorClass="text-violet-400"
        bookTitle="Mục Lục: Unified-App-Infra BaaS"
        onNavigateTab={onNavigateTab}
        testTabLabel="Luyện Thi Tình Huống Kỹ Sư Hạ Tầng"
        playTabLabel="Khám Phá Flashcards Thuật Ngữ Cốt Lõi"
      />
