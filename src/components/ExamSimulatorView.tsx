...
export const ExamSimulatorView: React.FC = () => {
  ...
  const handleSubmitAnswer = () => {
    ...
    const feedback = recordQuizAnswer(isCorrect, activeQuestion.difficulty);
    ...
    <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-6 md:py-8 space-y-6 md:space-y-8 text-slate-100">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-800 pb-5">
        <div>
          <div className="inline-flex items-center gap-2 text-xs font-semibold text-amber-400 bg-amber-500/10 px-3 py-1 rounded-full border border-amber-500/20 mb-2">
            ...
            Luyện Đề Song Ngữ (Tích Hợp Cơ Chế Điểm & XP Thông Minh)
          </div>
          <h1 className="text-xl md:text-3xl font-extrabold text-white">
            Trình Luyện Thi & Giải Tình Huống AWS
          </h1>
          <p className="text-xs md:text-sm text-slate-400 mt-1">
            Đúng nhận XP & Điểm xếp hạng + Chuỗi Bonus 🔥; Sai bị trừ điểm hợp lý để rèn luyện tính cẩn trọng.
          </p>
        </div>

        {/* Live Points / Streak Badge */}
        <div className="flex items-center gap-2">
          <div className="bg-slate-900 px-3 py-1.5 rounded-xl border border-slate-700 flex items-center gap-3 text-xs font-mono font-bold">
          ...
          >
            ...
            <span className="hidden sm:inline">Xáo Đáp Án</span>
          </button>
        </div>
      </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="bg-slate-900 p-0.5 rounded-lg border border-slate-800 flex items-center gap-0.5">
            <button
              ...
              className={`px-2.5 py-0.5 rounded text-[11px] font-bold ${mode === 'study' ? 'bg-amber-500 text-slate-950' : 'text-slate-400'}`}
            >
              Ôn Luyện
            </button>
            <button
              onClick={() => { setMode('exam'); handleRestartQuiz(); }}
              className={`px-2.5 py-0.5 rounded text-[11px] font-bold ${mode === 'exam' ? 'bg-sky-600 text-white' : 'text-slate-400'}`}
            >
              Thi Thử
            </button>
          </div>
          ...
          {mode === 'exam' && (
            <div className="flex items-center gap-1.5 font-mono text-amber-400 bg-amber-500/10 px-2.5 py-0.5 rounded-lg border border-amber-500/20">
      ...
      {!isExamCompleted && activeQuestion ? (
        <div className="bg-slate-900 rounded-3xl border border-slate-700/80 p-6 md:p-8 space-y-6 shadow-2xl">
          ...
          {/* Detailed Explanation with mapped correct letter & Gamification Reward Banner */}
          {mode === 'study' && isAnswerSubmitted && displayedExplanation && (
            <div className="space-y-4 pt-4 border-t border-slate-800 animate-fadeIn">
              
              {/* Correct / Incorrect Header with Dynamic XP / Points Badge */}
              <div className={`p-4 rounded-2xl flex flex-col sm:flex-row sm:items-center justify-between gap-3 ${
                ...
                {/* Score & XP Delta Badge */}
                {answerFeedback && (
                  <div className="flex items-center gap-2 self-start sm:self-auto font-mono text-xs font-bold">
                    {answerFeedback.pointsDelta > 0 ? (
                      <span className="bg-emerald-500/20 text-emerald-300 px-2.5 py-1 rounded-lg border border-emerald-500/30 flex items-center gap-1.5">
                        <TrendingUp className="w-3.5 h-3.5 text-emerald-400" />
                        <span>+{answerFeedback.xpDelta} XP</span>
                        <span>•</span>
                        <span>+{answerFeedback.pointsDelta} Điểm</span>
                        {answerFeedback.isStreakBonus && <span className="text-orange-400">🔥 Bonus!</span>}
                      </span>
                    ) : (
                      <span className="bg-red-500/20 text-red-300 px-2.5 py-1 rounded-lg border border-red-500/30">
                        {answerFeedback.pointsDelta} Điểm • 0 XP
                      </span>
                    )}
                  </div>
                )}
