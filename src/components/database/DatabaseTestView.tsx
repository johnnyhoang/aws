...
import { DATABASE_QUIZ_QUESTIONS } from '../../data/database/databaseQuizData';
import { DATABASE_FLASHCARDS } from '../../data/database/databaseFlashcardsData';
...
export const DatabaseTestView: React.FC = () => {
  ...
  // Flashcards State
  const [cards, setCards] = useState<DatabaseFlashcard[]>(DATABASE_FLASHCARDS);
  ...
  // Quiz Handlers
  const currentQ = DATABASE_QUIZ_QUESTIONS[currentQuizIdx] || DATABASE_QUIZ_QUESTIONS[0];
  ...
  const handleNextQuiz = () => {
    if (currentQuizIdx < DATABASE_QUIZ_QUESTIONS.length - 1) {
      ...
      <div className="flex items-center justify-between gap-3 pb-3 border-b border-slate-800 text-xs">
        <div className="flex items-center bg-slate-900 p-0.5 rounded-lg border border-slate-800">
          ...
          >
            ...
            <span>Đề Thi Trắc Nghiệm ({DATABASE_QUIZ_QUESTIONS.length} câu)</span>
          </button>
          <button
          ...
          >
            ...
            <span>Thẻ Nhớ Thuật Ngữ ({DATABASE_FLASHCARDS.length} thẻ)</span>
          </button>
        </div>
        ...
        <div className="space-y-6">
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between text-xs text-slate-400">
              <span className="font-mono text-amber-400 font-semibold">
                Câu {currentQuizIdx + 1} / {DATABASE_QUIZ_QUESTIONS.length} • {currentQ.category}
              </span>
          ...
          <div className="flex items-center justify-between pt-4 border-t border-slate-800 text-xs">
            ...
            <button
              ...
              disabled={currentQuizIdx === DATABASE_QUIZ_QUESTIONS.length - 1}
              className="p-2 rounded-md bg-slate-900 border border-slate-800 text-slate-400 hover:text-slate-200 disabled:opacity-30 disabled:cursor-not-allowed transition-colors cursor-pointer"
              title="Câu tiếp theo"
