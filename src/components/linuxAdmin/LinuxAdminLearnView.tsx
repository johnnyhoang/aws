...
import { LinuxAdminChapter, LinuxAdminCategory } from '../../types/linuxAdminModule';
import { 
  ChevronLeft, 
  ChevronRight, 
  Copy, 
...
const CATEGORY_NAMES: Record<LinuxAdminCategory, string> = {
  linux_fundamentals: 'Nền Tảng & Triết Lý',
  shell_command_line: 'Shell & Command Line',
  user_security_permissions: 'Phân Quyền & Bảo Mật',
  process_systemd_cron: 'Systemd & Tiến Trình',
  storage_fhs_lvm: 'Lưu Trữ FHS & LVM',
  networking_remote_ssh: 'Mạng Máy Chủ & SSH',
  web_app_docker_servers: 'Nginx, Docker & Web App',
  database_tuning_linux: 'Tối Ưu Database',
  aws_linux_administration: 'Linux Trên AWS EC2',
  monitoring_troubleshooting: 'Giám Sát & Xử Lý Sự Cố'
};
...
export const LinuxAdminLearnView: React.FC<LinuxAdminLearnViewProps> = ({ onNavigateTab }) => {
  ...
  const completedCount = LINUX_ADMIN_CHAPTERS.filter(ch => completedLessons.includes(ch.id)).length;
  const progressPercent = Math.round((completedCount / LINUX_ADMIN_CHAPTERS.length) * 100);
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 py-6 space-y-8 text-slate-300">
      ...
      <div className="space-y-3 pb-6 border-b border-slate-800">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="flex items-center gap-2 text-xs font-semibold text-amber-400 uppercase tracking-wider">
            ...
            <span>Sách Chuyên Khảo: Quản Trị Hệ Thống Linux & Unix Chuyên Gia</span>
          </div>
          ...
          <div className="flex items-center gap-3 text-xs bg-slate-900 border border-slate-800 px-3 py-1.5 rounded-full">
            ...
            <span className="font-mono font-bold text-amber-400">{completedCount}/{LINUX_ADMIN_CHAPTERS.length} ({progressPercent}%)</span>
          </div>
        </div>
            ...
            <div className="flex flex-wrap items-center justify-between gap-3">
              ...
              <div className="flex items-center gap-2 text-xs text-slate-400">
                ...
                >
                  ...
                  <span>Mục Lục ({LINUX_ADMIN_CHAPTERS.length})</span>
                </button>
      ...
      <BookTocDrawer
        ...
        chapters={LINUX_ADMIN_CHAPTERS}
        currentChapterId={currentChapter.id}
        completedLessonIds={completedLessons}
        onSelectChapter={handleSelectChapter}
        categoryNames={CATEGORY_NAMES}
        accentColorClass="text-amber-400"
        bookTitle="Mục Lục: Quản Trị Hệ Thống Linux"
        onNavigateTab={onNavigateTab}
        testTabLabel="Luyện thi tình huống & Thẻ nhớ Linux"
        playTabLabel="Mô phỏng Terminal Lab & Trình sinh cấu hình"
      />
