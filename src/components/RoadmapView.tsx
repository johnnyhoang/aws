    ...
    <div className="space-y-8 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 text-slate-100">
      ...
      <div className="relative overflow-hidden rounded-2xl bg-gradient-to-r from-slate-900 via-slate-800 to-slate-900 border border-slate-700 p-6 md:p-8 shadow-2xl">
        ...
        <div className="relative z-10 grid grid-cols-1 lg:grid-cols-3 gap-6 items-center">
          <div className="lg:col-span-2 space-y-3">
            ...
            <div className="pt-2 flex flex-wrap gap-2 text-xs text-slate-300">
              ...
              {currentTrackInfo.targetRoles.map((role, idx) => (
                <span key={idx} className="bg-slate-800/80 px-2.5 py-1 rounded-md border border-slate-700 text-slate-200">
                  {role}
                </span>
              ))}
            </div>
          ...
          {/* Quick CTA Box */}
          <div className="bg-slate-800/90 rounded-xl p-5 border border-slate-700 space-y-3 shadow-lg">
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-slate-400">Chọn từng chứng chỉ bên dưới để xem chi tiết miền kiến thức, mẹo thi và khóa học đề xuất</p>
          </div>
        </div>
          ...
          {filteredStages.map((stage, idx) => {
            ...
            return (
              <div
                ...
                className={`cursor-pointer rounded-xl p-5 border transition-all duration-200 relative overflow-hidden flex flex-col justify-between ${
                  isSelected
                    ? 'bg-slate-800 border-amber-500 ring-2 ring-amber-500/20 shadow-xl shadow-amber-500/10'
                    : 'bg-slate-900/80 border-slate-800 hover:border-slate-700 hover:bg-slate-800/50'
                }`}
              >
                {/* Step indicator */}
                <div className="flex items-center justify-between mb-3">
                  <span className="text-[11px] font-bold uppercase tracking-wider px-2 py-0.5 rounded bg-slate-800 text-slate-400 border border-slate-700">
                ...
                <div>
                  <div className="flex items-center gap-2">
                    <span className="text-lg font-black text-amber-400 tracking-tight">{stage.code}</span>
                    <span className="text-xs px-2 py-0.5 rounded bg-slate-800 text-slate-300 font-medium">
                      ~{stage.estimatedWeeks} tuần
                    </span>
                  </div>
                  <h3 className="font-bold text-white text-sm mt-1 line-clamp-1">{stage.name}</h3>
                ...
                <div className="mt-4 pt-3 border-t border-slate-800/80 flex items-center justify-between text-xs text-slate-400">
                  ...
                  <span className="text-slate-500">{stage.mustKnowServices.length} dịch vụ cốt lõi</span>
                </div>
              </div>
            );
          })}
      ...
      {/* Selected Certificate In-depth Panel */}
      {selectedStage && (
